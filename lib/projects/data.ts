...
export const PROJECTS: Project[] = [
  {
    ...
    features: [
      { title: "Triển khai từ Docker Image", description: "Cung cấp tên image, hệ thống tự động tạo Deployment, Service, Ingress và cấp phát URL truy cập chỉ qua vài thao tác trên giao diện.", icon: "Deployment" },
      { title: "Build từ GitHub — Không cần Dockerfile", description: "Cloud Native Buildpacks tự động nhận diện ngôn ngữ (Node.js, Java, Python, Go, PHP), build container image chuẩn OCI và triển khai lên cụm.", icon: "Build" },
      { title: "Scaffold dự án Microservices", description: "Chọn Blueprint (Next.js + Express + PostgreSQL), hệ thống tự động tạo GitHub repos, cấp database, build image và triển khai theo DAG 3 tầng: DB → API → Frontend.", icon: "Scaffold" },
      { title: "Quản trị cụm Kubernetes qua giao diện", description: "Khởi tạo cluster, thêm node, cài đặt hạ tầng (Kpack, Redis, Metrics Server, Prometheus), cordon/drain/remove node — tất cả qua web UI, không cần SSH.", icon: "Cluster" },
      { title: "Cân bằng tải & Scale tự động", description: "Scale replica qua giao diện, Kubernetes phân phối request đều giữa các Pod trên nhiều node. Kịch bản 500 request cho kết quả 167/167/166.", icon: "LoadBalancer" },
      { title: "Xử lý bất đồng bộ với BullMQ + DAG", description: "Tác vụ triển khai được đẩy vào hàng đợi BullMQ, xử lý bởi worker riêng biệt. BullMQ Flow điều phối tác vụ có phụ thuộc theo cấu trúc DAG.", icon: "Queue" },
    ],
    techStack: [
    ...
    environment: [
      { name: "Host Machine", ip: "192.168.56.13", role: "Backend API (NestJS), Frontend (Next.js), PostgreSQL", status: "active" },
      { name: "Master VM (Control Plane)", ip: "192.168.56.10", role: "Kubernetes Control Plane, Traefik Ingress, Local Registry, Kpack (Build service)", status: "active" },
      { name: "Worker VM", ip: "192.168.56.11", role: "Application workloads (Pods), Metrics Server, Prometheus/Grafana", status: "active" },
    ],
  },
  {
    slug: "laptop-shop",
    ...
    features: [
      { title: "Sub-second Page Loads", description: "ISR và edge caching đảm bảo tốc độ tải trang dưới 1 giây ngay cả ở peak traffic.", icon: "FastLoad" },
      { title: "Smart Inventory Sync", description: "Realtime inventory tracking với tự động cảnh báo và reorder suggestion dựa trên AI.", icon: "Sync" },
      { title: "SEO Architecture", description: "Structured data, canonical URLs, và sitemap tự động giúp tăng organic search traffic.", icon: "SEO" },
    ],
    techStack: [
      { name: "Next.js", category: "frontend" },
  ...
  },
  {
    ...
    features: [
      { title: "Visual Search", description: "Upload ảnh để tìm sản phẩm tương tự trong toàn bộ catalog với độ chính xác cao.", icon: "VisualSearch" },
      { title: "Semantic Search", description: "Tìm kiếm bằng ngôn ngữ tự nhiên — hỏi như hỏi người thật, nhận kết quả chính xác.", icon: "SemanticSearch" },
      { title: "Personalised Feed", description: "AI học từ hành vi người dùng để cá nhân hóa trải nghiệm mua sắm theo thời gian.", icon: "Personalized" },
    ],
    techStack: [
  ...
  },
  {
    ...
    features: [
      { title: "Module Graph Inspector", description: "Trực quan hóa realtime toàn bộ dependency graph, circular dependencies và lazy modules.", icon: "ModuleGraph" },
      { title: "Route Analyser", description: "Kiểm tra tất cả HTTP routes, guards, interceptors và middlewares đang hoạt động.", icon: "RouteAnalyzer" },
      { title: "Provider Registry", description: "Xem toàn bộ providers, scope và injection tokens trong runtime context.", icon: "ProviderRegistry" },
    ],
    techStack: [
      { name: "NestJS", category: "backend" },
      { name: "TypeScript", category: "backend" },
  ...
  },
  {
    ...
    features: [
      { title: "Universal Bridge", description: "Kết nối bất kỳ AI framework (LangChain, CrewAI, AutoGen) với external services qua một interface thống nhất.", icon: "UniversalBridge" },
      { title: "Context Synapse", description: "Shared memory layer giúp agents nhớ và chia sẻ context across sessions và tools.", icon: "ContextSynapse" },
      { title: "Smart Tool Routing", description: "Tự động chọn và chain tools dựa trên agent intent, không cần hardcode workflows.", icon: "SmartRouting" },
    ],
    techStack: [
      { name: "Python", category: "backend" },
      { name: "TypeScript", category: "backend" },
